...
**而存储引擎层负责数据的存储和提取。其架构模式是插件式的，支持 InnoDB(默认)、MyISAM（不支持事务）、Memory （针对具体表，而非数据库）等多个存储引擎。现在最常用的存储引擎是 InnoDB，它从 MySQL 5.5.5 版本开始成为了默认存储引擎。**
...
对于有**索引**的表，执行的逻辑也差不多。第一次调用的是“取满足条件的第一行”这个接口，之后循环取“满足条件的下一行”这个接口，这些接口都是引擎中已经定义好的。





转自 ：MySQL实战45讲 -01 | 基础架构：一条SQL查询语句是如何执行的？ 林晓斌



总结：对mysql中的一条sql执行过程有了初步理解，连接器-》分析器-》查询缓存（8.0去掉）-》优化器-》执行器-》存储引擎 整个过程在脑海里有了整体关系图。然后其中各个组件的作用及相互关系。

现在好奇Oracle数据库中一条SQL的执行过程。
